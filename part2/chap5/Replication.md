...
failover 과정에서 여러 오류가 발생할 수 있다. 먼저 비동기 복제방식의 경우, 새로운 리더는 이전 리더에게 모든 쓰기작업을 수행하지 않을수 있는데, 이러한 경우 이전 리더의 상태와 일관성이 깨지게 된다. 보통 이러한 경우 이전리더에 있던 반영되지 않았던 쓰기작업은 버려지게 되는데, 이는 클라이언트의 durability 기대에 어긋난다.
...
- `NOW()`나 `RAND()`와 같이 nondeterministic한 함수르 호출하는 경우 쿼리결과가 달라진다.
...
이 방식의 문제점은 WAL과 스토리지 엔진이 강하게 커플링이 되어있다. 그래서 스토리지 엔진 버전이 일치하지 않으면 follower에서 해당 WAL을 제대로 읽을수 없다는 문제가 있다. 이럴경우, 스토리지 엔진을 업데이트해야할수도 있는데 이경우 디비에 downtime이 발생한다.
...
- 유저가 수정한 시간을 기록하여 최소한 해당 시간까지는 해당 리플리카에 싱크된 데이터를 받아오는 방법
